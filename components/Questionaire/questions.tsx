...
const Questions = ({ setMode, quizMode = 'fullburst', maxQuestions }: QuestionsProps) => {
  const { data, isLoading, error, handleGetData, isInitialLoading } = useAppProvider();
  const currentQuestionNumber = useRef(1); // Track current question number
  const [selectedOption, setSelectedOption] = useState<number | null>(null);
  const [isTransitioning, setIsTransitioning] = useState(false);
  const [showResult, setShowResult] = useState(false);
  const [isCorrect, setIsCorrect] = useState<boolean | null>(null);
  ...
  // Load initial data on component mount - Modified for orderly questions
  useEffect(() => {
    console.log('Questions useEffect triggered', { hasLoadedInitial, isInitialLoading });
    ...
    if (!hasLoadedInitial) {
      ...
      const loadInitialData = async () => {
        try {
          // Start with question 1 for orderly progression
          const params = quizMode === 'random10'
            ? { question_number: 1, mode: 'sequential', max_questions: maxQuestions || 10 }
            : { question_number: 1 };
  ...
  }, [hasLoadedInitial, quizMode, maxQuestions]);
  ...
  const getData = async () => {
    ...
    setTimeout(async () => {
      ...
      // If we exceed total questions, loop back to 1 (or handle as needed)
      if (data?.total_questions && currentQuestionNumber.current > data.total_questions) {
        currentQuestionNumber.current = 1;
      }

      const params = quizMode === 'random10'
        ? { question_number: currentQuestionNumber.current, mode: 'sequential', max_questions: maxQuestions || 10 }
        : { question_number: currentQuestionNumber.current };
  ...
  const handleRestart = () => {
    ...
    // Reset question counter and state
    setTimeout(async () => {
      currentQuestionNumber.current = 1; // Reset to first question
      // Reset score tracking
      setScore(0);
      setTotalQuestions(0);
      setQuestionsCompleted(0);
        ...
        <div className="flex min-h-dvh items-center justify-center p-2">
          <div className="text-center max-w-4xl w-full">
            ...
            <div className="mb-8 animate-bounce">
              ...
              <div className="text-xl text-gray-600">
                You've completed the {quizMode === 'random10' ? 'Sequential 10' : 'Full Sequential'} quiz!
              </div>
            </div>
        ...
        <div className="text-center max-w-4xl w-full">
          ...
          <div className={`mb-8 transition-all w-full duration-300 ease-in-out transform ${isTransitioning
            ...
            <div className="flex items-center justify-center mb-6 relative">
              ...
              <PiBrainThin className="text-4xl lg:text-6xl text-gray-600 mr-4" />
              <div className="lg:text-lg text-sm font-medium text-gray-500">
                Question {currentQuestionNumber.current}
                <div className="text-sm text-gray-400">
                  {quizMode === 'random10' ? 'Sequential 10' : 'Sequential'} Mode
                </div>
              </div>
          ...
          <div className="mt-6">
            <div className="text-sm text-gray-500 mb-2">
              Question Progress ({currentQuestionNumber.current} of {effectiveMaxQuestions})
            </div>
            <div className="w-full max-w-md mx-auto bg-gray-200 rounded-full h-2">
              <div
                className="bg-gray-600 h-2 rounded-full transition-all duration-500 ease-out"
                style={{ width: `${(currentQuestionNumber.current / effectiveMaxQuestions) * 100}%` }}
              ></div>
            </div>
            <div className="text-xs text-gray-400 mt-1">
              {Math.round((currentQuestionNumber.current / effectiveMaxQuestions) * 100)}% Complete
            </div>
          </div>
        </div>
      </div>
